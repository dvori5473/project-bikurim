...
const login = async (req, res) => {
  ...
  if (!foundUser || !foundUser.active) {
    ...
    return res.status(401).json({ messages: 'Unauthorized1' })
  }
  const match = await bcrypt.compare(password, foundUser.password)
  if (!match) return res.status(401).json({ messages: 'Unauthorized' })
  // const userToken = { _id: foundUser._id, firstName: foundUser.firstName, lastName: foundUser.lastName, roles: foundUser.roles,phone:foundUser.phone, user_id: foundUser.user_id, email: foundUser.email }
  const userInfo = {
    _id: foundUser._id,
...
const register = async (req, res) => {
  ...
  if (duplicate) {
    return res.status(409).json({ messages: 'Duplicate usrName' })
  }
